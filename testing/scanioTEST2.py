...
from multiprocessing import Process, set_start_method, Pool, Value, Lock, Manager
...
class scanjobs(object):
    ...
    def start(self, _secs, scanType, net, hosts, ports, timeout, currcount, robust, progtext):
        window = _window()
    ...
    def callScanNC(self, timeout, currcount, q, robust, addys):
        ...
        if "open" in result or "succ" in result:
            ...
            #Banner Grab
            if robust == '':
                banner = self.bannerGrab(timeout, addy, tp)
            else:
            ...
            io.sortXML(io(), addy)
            # q.put(str(addy)+':'+str(tp)+':'+str(banner)+':'+str(robust))
            # print(result)
    ...
    def bannerGrab(self, timeout, addy, port):
        try:
...
class io(object):
    ...
    def process_results(self, queue, currcount, totalcount, progtext, net):
        ...
        while currcount.value < totalcount:
            # try:
            results = q.get(True)
            addy = results.split(':')[0]
            # net = '{0}.{1}.{2}'.format(addy.split('.')[0],addy.split('.')[1],addy.split('.')[2])
            tp = results.split(':')[1]
            banner = results.split(':')[2]
            robust = results.split(':')[3]
            # printext = printaddy
            # print('\n{0}\n'.format(printext))
            

            # with term.location(3,0):
            #     print(term.clear_eos)
            #     print('{0}\n\n'.format(progtext.value))
...
secs = Value('i', 0)
flag = Value('i', 0)
progressVar= Value(ctypes.c_wchar_p, ' ')

if __name__ == '__main__':
    scan = scanjobs()
    write = io()
    show = display()
    scanInfo = scan.initiate()
