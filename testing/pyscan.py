...
def addHost(addy):
    with lock:
        sl = addy.split('.')
        subnetstr = './subnet[@address = "{0}.{1}.{2}"]'.format(sl[0], sl[1], sl[2])
        tree = ET.parse('pyscan.xml')
        root = tree.getroot()
...
def initiate():
    ...
    uinput.add_argument("-n", "--new", help = "Clears the log and starts fresh.", action="store_true")
    uinput.add_argument("-f", "--fast", help = "Performs a fast scan using netcat vs the default /dev/tcp.  This option does have \
                                                ...
                                                ", action="store_true")
    uinput.add_argument("--show", help = "Shows the currently logged results for the address.  Nothing more", action="store_true")
    ...
    return operation, net, final_range, final_ports, totalscans, printnet
...
def sortXML(addy):
    ...
    tree = ET.parse('pyscan.xml')
    root = tree.getroot()
    subnetstr = './subnet[@address = "'+naddy+'"]'
    subnet = root.find(subnetstr)
...
def printall(addy):
    if len(addy.split('.')) > 3:
        ...
        if subnethosts:
            ...
            print('\n---------------')
            print(ip)
            for p in root.findall('./subnet[@address = "'+naddy+'"]/host/[address = "'+ip+'"]/port/number'):
                # print('     {0}'.format(p.text))
                plist.append(int(p.text))
            for pp in sorted(plist):
                printtext = '|__ {0}'.format(pp)
                for b in root.findall('./subnet[@address = "'+naddy+'"]/host/[address = "'+ip+'"]/port/[number = "'+str(pp)+'"]/banner'):
                    if b.text:
                        banner = b.text
        ...
        if subnethosts:
            hlist = list()
            plist = list()
            for h in root.findall('./subnet[@address = "'+naddy+'"]/host/address'):
                hlist.append(h.text)
